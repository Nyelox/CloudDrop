...
class WorkerThread(threading.Thread):
    ...
    def handle_login(self, data):
        ...
        try:
            ...
            # Check if response is a dict (new format) or string (old format fallback just in case)
            if isinstance(response, dict):
                if response.get("status") == "success":
                    self.signals.success.emit(username, response.get("is_admin", False))
                else:
                    self.signals.error.emit(response.get("message", "Login failed"))
            # Fallback for string (should not happen if backend updated correctly)
            elif response == "Login successful":
                self.signals.success.emit(username, False)
            else:
